...
int config_parse_dnssd_service_name(const char *unit, const char *filename, unsigned line, const char *section, unsigned section_line, const char *lvalue, int ltype, const char *rvalue, void *data, void *userdata) {
        ...
        free(s->name_tpl);
        s->name_tpl = strdup(rvalue);
        if (!s->name_tpl)
                return log_oom();

        r = dnssd_render_instance_name(s, &name);
        if (r < 0) {
                log_syntax(unit, LOG_ERR, filename, line, 0, "Failed to render to valid instance name '%s'. Ignoring.", name);
                return -EINVAL;
        }
